...
def ploteame(txt, Comunidad, tipo, densidad):
    ...
    df.sort_values(by=df.columns[len(df.columns)-1],
    ...
    for i in range(len(Comunidad)):
        ...
        if tipo.upper().find('DIA') > -1:
           # ax.bar(df.iloc[i].index, df.iloc[i].diff(), color = cm.tab10(i), label = NombreComunidad, alpha = 0.8)
            ax.plot(df.iloc[i].index, df.iloc[i].diff(),
                    label=NombreComunidad,  color=cm.tab10(i), linewidth=4)

        elif tipo.upper().find('TOT') > -1:
            ax.plot(df.iloc[i].index, df.iloc[i],
                    label=NombreComunidad,  color=cm.tab10(i), linewidth=4)
...
def GetStat(txt):
    if txt == 'FALLECIDOS':
        ...
        url = "https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/ccaa_covid19_fallecidos_por_fecha_defuncion_nueva_serie.csv"
    ...
    if txt == 'FALLECIDOS':
        ...
        Esp = Stat.sum()
        Esp.name = (0, 'España')
        Stat = Stat.append(Esp)
        Stat.columns = pd.to_datetime(Stat.columns)
        Stat = Stat.cumsum(axis=1)

    if txt == 'INFECTADOS':
        table = pd.pivot_table(Stat, values='num_casos', index=['cod_ine', 'ccaa'],
                               columns=['fecha'], aggfunc=np.sum)
        Esp = table.sum()
        Esp.name = (0, 'España')
        Stat = table.append(Esp)
        Stat.columns = pd.to_datetime(Stat.columns)
        Stat = Stat.cumsum(axis=1)
...
def keywordDetector(txt):
    ...
    if txt == '/start':
        resp = 'Hola, soy el coronabot!\n\nTengo datos actualizados del número de contagiados y fallecidos por COVID-19 ' +\
            ...
            'También puedes ver las comunidades més o menos afectadas usando la palabra Top.' +\
            ' Por ejemplo: \n\n -_Top 7 de fallecidos acumulados_: Obtendrás las 7 comunidades con más fallecidos\n\n' +\
               '-_Top -3 de contagiados acumulados por millon_: Obtendrás las 3 comunidades con menos afectados por millón.\n\nSi no hay número después de Top, será 5' +\
            ' por defecto y como máximo se enseñarán 8 comunidades distintas!\n\n' +\
    ...
    if count == 0:
        ...
        resp = 'Me faltan los siguientes datos:\n\n' + answer[0] + answer[1] + 'Escribelo todo en un mismo mensaje, por favor.\n' +\
               'Te recuerdo que si añades *"millon"* te daré los datos normalizados.'
        return resp
    else:
        resp = 'No te he entendido! Recuerda que funciono con palabras clave, así que dime en una misma frase qué quieres saber: \n\n ' +\
            ...
            '\* Además, si dices *"millon"* te daré los *datos normalizados* por cada millón de habitantes. \n\n' + \
            'Ejemplo de frase:\n\n_Numero de infectados acumulados en España, Madrid, Aragon y Andalucia_\n\n' +\
            'También puedes ver las comunidades mas o menos afectadas usando la palabra Top. ' +\
            'Por ejemplo: \n\n -_Top 7 de fallecidos acumulados_: Obtendrás las 7 comunidades con más fallecidos\n\n' +\
            '-_Top -3 de contagiados acumulados por millón_: Obtendrás las 3 comunidades con menos afectados por millón.\n\nSi no hay número después de Top, será 5' +\
            ' por defecto, y como máximo se enseñarán 8 comunidades distintas!\n\n' +\
            'No me importa en qué orden me digas las cosas. Simplemente necesito las tres primeras palabras clave y funcionaré!\n\n' +\
            'Muchas gracias por usarme!'
